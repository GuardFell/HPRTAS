...
| PB-006 | Create the Camunda Forms | Forms for referral, clinical review and funding or payment. | Must | FR-002, FR-004, FR-018 | 8 | M4 | M2 | PB-002, PB-003 | Bound to the correct tasks with validation; the variables are exchanged correctly. | `forms/` | Done - the eight forms are bound and the bindings resolve; 36 of the 55 user tasks still bind no form (`DEF-13`). |
